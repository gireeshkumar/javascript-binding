...
// object.watch
function checkAddPropertyWatch(objinstance){
	if (!objinstance.__proto__.$gwatch) {
		objinstance.__proto__.$gwatch = function(prop, handler) {
			...
			if (delete this[prop]) { // can't watch constants
				if (objinstance.defineProperty) { // ECMAScript 5
					objinstance.defineProperty(this, prop, {
						get : getter,
						set : setter
					});
				} else if (objinstance.__proto__.__defineGetter__
						&& objinstance.__proto__.__defineSetter__) // legacy
				{
					objinstance.__proto__.__defineGetter__.call(this, prop, getter);
					objinstance.__proto__.__defineSetter__.call(this, prop, setter);
				}
			}
		};
	}
	...
	// object.unwatch
	if (!objinstance.__proto__.$gunwatch) {
		objinstance.__proto__.$gunwatch = function(prop) {
			var val = this[prop];
...
//						        var x = value.attr("value");
			  ...
			  if( !(eventhandler[idname][propname])){
				  checkAddPropertyWatch(window[idname]);
				  window[idname].$gwatch(propname, function(){
